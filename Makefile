...
TARG.Name:=runtime-provider-aws
TRAG.Gopkg:=openpitrix.io/runtime-provider-aws
...
.PHONY: build
build: fmt ## Build all runtime-provider-aws images
	mkdir -p ./tmp/bin
...
.PHONY: compose-up
compose-up: ## Launch runtime-provider-aws in docker compose
	docker-compose up -d
